...
class ShowCourseController extends Controller
{
    protected function allActiveCourse(Request $request){
        ...
        $courses=Course::leftJoin('categories','courses.category_id','categories.id')
            ...
            ->select('categories.category_name','sub_category.sub_category_name','courses.name','courses.url','courses.overview','courses.image_small','courses.start_date')
    ...
    protected function singleCourseDetails($courseUrl){
        $course=Course::where(['url'=>$courseUrl,'status'=>1])->first();
        if (empty($course)){
